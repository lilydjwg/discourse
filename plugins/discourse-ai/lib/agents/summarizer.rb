    ...
    class Summarizer < Agent
      ...
      def system_prompt
        <<~PROMPT.strip
          ...
          - Cite specific noteworthy posts using the format [DESCRIPTION](URL_PREFIX/POST_NUMBER)
          - Example: link to the 6th post by jane: [agreed with](URL_PREFIX/6)
          - Example: link to the 13th post by joe: [joe](URL_PREFIX/13)
          - When formatting usernames use [USERNAME](URL_PREFIX/POST_NUMBER)
      ...
      def examples
        [
          [
            ...
            {
              summary:
                "The discussion covers how users can reset their password when they no longer remember which email address is tied to their account. [chloe_dev](/t/-/123/1) explains they registered with an email alias they can't recall, and the reset form only accepts an email address. [marco](/t/-/123/3) points out this is controlled by the `hide email address taken` site setting: when enabled (the default), only emails are accepted to prevent account enumeration, while disabling it allows resets by username.\n\nAfter [confirming the workaround](/t/-/123/4), chloe_dev argues that username-based resets should always be possible since forgetting a per-site alias is common. [tobias_m](/t/-/123/6) warns that disabling the setting lets anyone probe which emails are registered and recommends contacting staff instead, and [marco](/t/-/123/7) notes an open feature request to allow username-based resets without leaking account information.",
            }.to_json,
          ],
        ]
      end
